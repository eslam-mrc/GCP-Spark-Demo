...
# setting spark.sql.autoBroadcastJoinThreshold to force spark to use shuffle sort merge join
spark.conf.set("spark.sql.autoBroadcastJoinThreshold", -1)    

# saving DFs as managed tables by bucketing them in Parquet format
gsmaDF.orderBy("device_tac").write.format("parquet").bucketBy(8,"device_tac").saveAsTable("gsmaTable")
radioDF.orderBy("device_tac").write.format("parquet").bucketBy(8,"device_tac").saveAsTable("radioTable")

# caching the tables
spark.sql("""CACHE TABLE gsmaTable """)
spark.sql("""CACHE TABLE radioTable """

# reading cached tables back in
newGSMA_DF = spark.table("gsmaTable")
newRadio_DF = spark.table("radioTable")

# joining the newRadio_DF and newGSMA_DF while selecting the needed columns only
newJoinedDF = newRadio_DF.join(newGSMA_DF, "device_tac").select(newRadio_DF["*"], newGSMA_DF["device_manufacturer"])
...
outputDF = newJoinedDF.withColumn("row_id",row_number().over(windowSpec)).select("*").where(col("row_id")<=10)
